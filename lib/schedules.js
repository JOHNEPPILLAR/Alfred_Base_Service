...
const debug = require('debug')('Base:Schedule');
...
function activateSchedules() {
  try {
    ...
    debug('Set daily reset schedule up for tomorrow');
...
function addSchedule(hour, minute, description, functionToCall, ...rest) {
  try {
    ...
    debug(`Add ${description} schedule to schedules array`);
    this.logger.info(
