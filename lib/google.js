...
async function _getGoogleCal(query, callID) {
  try {
    ...
    const filteredItems = events.data.items.filter(
      (item) => item.summary === query,
    );
...
/**
 * Check google calendar to see if at home
 */
async function _atHomeToday() {
  debug('Check if at home today');
  try {
    const events = await this._getGoogleCal('OFFICE', 'JPGoogleAPICalendarID');
...
/**
 * Check google calendar to see if on holiday
 */
async function _onHolidayToday() {
  debug('Check if on holiday today');
  try {
    const events = await this._getGoogleCal('HOLIDAY', 'JPGoogleAPICalendarID');
    if (events instanceof Error) return events;

    // Process calendar events
    if (events.length > 0) {
      debug('Not on holiday today');
      return true;
    }
    debug('On holiday today');
    return false;
  } catch (err) {
    this.logger.error(`${this._traceStack()} - ${err.message}`);
    return err;
